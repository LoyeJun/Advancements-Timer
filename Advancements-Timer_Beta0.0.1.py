...
class display:
    def text(screen, text:str, size:int, colour:tuple, pos:tuple, centered:bool, max_length=9999999):
        size = int(size)
        text = str(text)
        font = pygame.font.Font(get_path('assets/unifont-16.0.01.otf'), size)
        text_surface = font.render(text, False, colour)
        text_rect = text_surface.get_rect()
        if text_rect.width > max_length:
            display.text(screen, text[:-2]+'…', size, colour, pos, centered, max_length)
        else:
            if centered:
    ...
    def image(screen, image, alpha:int, size:tuple, pos:tuple, centered:bool, angle=0):
...
os.environ['SDL_VIDEO_WINDOW_POS'] = "{},{}".format(x0-x1, y0)
